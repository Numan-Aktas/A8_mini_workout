...
class SIYISDK:
    ...
    def setGimbalRotation(self, yaw, pitch, err_thresh=1.0, kp=4):
        ...
        if pitch >= 45 or pitch <= -135 :
            self._logger.error("pitch sınırları dışında değer")
            return

        if yaw >= 160 or yaw <=-160 :
            self._logger.error("yaw Sınırları dışında değer")
            return
...
def information():
    ...
    print("request Gimbal Attitude: ",cam.requestGimbalAttitude())
    print("Zoom Level: ", cam.getZoomLevel())
    print("Attitude: ", cam.getAttitude())
    print("buffer Callback: ", cam.bufferCallback())
    print("check Connection: ", cam.checkConnection())
    print("Attitude Speed: ", cam.getAttitudeSpeed())
    print("Hardware ID: ", cam.getHardwareID())
